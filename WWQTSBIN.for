...
C WRITE TO ASCII FILE WQWCTS.OUT HERE:
            IF(ISASCII)THEN
             WRITE(1,71) IL(LL),JL(LL),K,TIMTMP, CHLWQ,TOCWQSUM(LL,K),
     +       WQVSUM(LL,K,6), TPWQSUM(LL,K), WQVSUM(LL,K,9),
     +       WQVSUM(LL,K,10), PO4DWQSUM(LL,K), CHLC, TNWQSUM(LL,K),
     +       (WQVSUM(LL,K,NW),NW=13,15), TSIWQSUM(LL,K),
     +       (WQVSUM(LL,K,NW),NW=16,17), SADWQSUM(LL,K), BOD5SUM(LL,K),
     +       WQVSUM(LL,K,19), CHLD, CHLG, WQVSUM(LL,K,NWQV)
            ENDIF
...
C
        IF(ISCOMP .GT. 0)THEN
          ...
          OPEN(UNIT=2, FILE='WQDOCOMP.BIN', ACCESS='DIRECT',
     +     FORM='UNFORMATTED', STATUS='UNKNOWN', RECL=MAXRECL3)

          READ(2, REC=1) NDUM, XDUM, XDUM,
     +      XDT, IXDT, NPARM, NCELLS, NLAYERS
          WRITE(2, REC=1) NREC3, TBEGAN, TIMTMP,
     +      XDT, IXDT, NPARM, NCELLS, NLAYERS

          WRITE(2, REC=NR5) TIMTMP
          DO LL=2,LA
            DO K=1,KC
...
C
              WRITE(2) XLIMNC(LL,K), XLIMND(LL,K), XLIMNG(LL,K),
     ...
     +        XDOSAT(LL,K), XDOPSL(LL,K), XDOSOD(LL,K),
