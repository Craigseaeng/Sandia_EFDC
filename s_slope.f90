SUBROUTINE SEDZLJ_SLOPE
  ...
  ! For Beta Angle "Pitch" and Theta Angle "Roll"
    IF(SUB(L+1)/=0.AND.SUB(L)/=0.AND.SUB(L-1)/=0)THEN           !Central differencing
      DELZX=BELV(L+1)-BELV(L-1)                      
      XDIST=0.5*DXP(L+1)+DXP(L)+0.5*DXP(L-1)                    !Across 3 cells
    ELSEIF(SUB(L+1)==0.AND.SUB(L)/=0.AND.SUB(L-1)/=0)THEN       !Forward differencing
      DELZX=BELV(L)-BELV(L-1)
      XDIST=0.5*(DXP(L)+DXP(L-1))                               !Across 2 cells
    ELSEIF(SUB(L+1)/=0.AND.SUB(L)/=0.AND.SUB(L-1)==0)THEN
      DELZX=BELV(L+1)-BELV(L)
      XDIST=0.5*(DXP(L+1)+DXP(L))                               !Across 2 cells
    ELSE
