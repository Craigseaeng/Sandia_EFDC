...
C *** DSLLC BEGIN BLOCK  
C  
      IF(JSWAVE.EQ.1) GOTO 100  ! JSWAVE set to one after first read
      JSWRPH=1  
      DO L=1,LC  
        HMPW(L)=0.  
        HMCW(L)=0.  
     ...
     &    NTSWV,WVDISV,WVDISH,WVLSH,WVLSX,ISWVSD,ISDZBR  
      ...
      WVFRQ=2.*PI/WVPRD  
!      NWCUNT=NWUPDT-1  
      JSWAVE=1  !  Set equal to 1 the first time this has been read
      JSSWAVE=1  ! Counter that becomes 2 after second read
      WREADCNT=1 ! Counter set to 1 after first read
...
C  
  100 CONTINUE  !  Come directly here after first read
      NWCUNT=NWCUNT+1  
      ...
      NWCUNT=0  
      GDFRQ=G/WVFRQ  
      !  DO NOT READ FIRST TIME THROUGH
      
!      IF(JSSWAVE.EQ.1) GOTO 200
      
!      JSSWAVE=2
      
      PRINT *,'WAVE: WAVE.INP'  ! Open file and scroll through to next dataset
      OPEN(1,FILE='WAVE.INP',STATUS='UNKNOWN')  
      DO NSKIP=1,31+((NWVDAT+1)*WREADCNT)  
        READ(1,1,IOSTAT=ISO)  
        IF(ISO.GT.0) GOTO 1081  
      ENDDO  
      
      WREADCNT=WREADCNT+1  
      READ(1,*,IOSTAT=ISO)WVPRD
      WVFRQ=2.*PI/WVPRD  
      NWCUNT=0 
