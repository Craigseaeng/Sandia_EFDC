SUBROUTINE SHOWVAL
  ...
  IF(NSHOWC.GE.NSHOWR)THEN
    ...
    IF(NSHTYPE==1)THEN
      ...
      WRITE(*,'(A)')'-------------------------------------------------------------------------------'
  IF(MAXVAL(MVEGL(:))>90.AND.TIME>0.01)WRITE(*,'("SUPPORT ENERGY LOSS:              ",F20.3," MW-hr")')SUM(ESUP(:,:))
  IF(MAXVAL(MVEGL(:))>90.AND.TIME>0.01)WRITE(*,'("MHK ENERGY REMOVAL:               ",F20.3," MW-hr")')SUM(EMHK(:,:))
  IF(MAXVAL(MVEGL(:))>90.AND.TIME>0.01)WRITE(*,'("MHK POWER REMOVAL (INSATNTANEOUS):",F20.3," MW")')SUM(PMHK(:,:))*1E-6
