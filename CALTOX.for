...
C
C CHANGE RECORD  
C **  SUBROUTINE CALTOX CALCULATES NONCOHESIVE SEDIMENT SETTLING,  
...
C  
      ...
      ENDIF  
C  
C **  CALCULATE TOXIC CONTAMINANT PARTICULATE FRACTIONS IN WATER COLUMN
C
...
C                         USED AS TEMPORARY VARIABLE IN THIS AND  
...
C  
C PARTITION TO COHESIVE SEDIMENT 
C  
            DO NS=1,NSED  
     ...
     &                  *STFPOCW(L,K,NS)*TOXPARW(NS,NT)  
          ...
          ENDIF  
C  
C PARTITION TO NONCOHESIVE SEDIMENT
C  
...
C  
        ...
        DO NT=1,NTOX  
          DO K=1,KC  
            DO L=2,LA  
              TOXFDFW(L,K,NT)=1.0/(1.0+TOXPFTW(L,K,NT))
...
C  
     ...
     &             SUB(L+1)*MAX(QSBDLDX(L+1,NX),0.0)*TMPTOXC  
     &            +SUB(L+1)*MIN(QSBDLDX(L+1,NX),0.0)*TMPTOXE  
     &            -SUB(L  )*MAX(QSBDLDX(L  ,NX),0.0)*TMPTOXW  
     &            -SUB(L  )*MIN(QSBDLDX(L  ,NX),0.0)*TMPTOXC  
     &            +SVB(LN )*MAX(QSBDLDY(LN ,NX),0.0)*TMPTOXC  
     &            +SVB(LN )*MIN(QSBDLDY(LN ,NX),0.0)*TMPTOXN  
     &            -SVB(L  )*MAX(QSBDLDY(L  ,NX),0.0)*TMPTOXS  
     &            -SVB(L  )*MIN(QSBDLDY(L  ,NX),0.0)*TMPTOXC)
            ENDIF  
          ENDDO  
        ENDDO  
      ENDDO  
...
C  
      DO NT=1,NTOX  
        DO L=2,LA  
          ...
          TOXF(L,0,NT)=TOXF(L,0,NT)+TMPVAL*(1.0-TOXPFTW(L,1,NT))  
        ENDDO  
      ENDDO  
      DO NT=1,NTOX  
        DO L=2,LA  
