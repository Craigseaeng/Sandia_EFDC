...
SUBROUTINE SEDZLJ_MAIN
  USE GLOBAL
  IMPLICIT NONE
!  DOUBLE PRECISION,DIMENSION(LCM)::WVEL,CLEFT,CRIGHT,GRADSED,SEDAVG,CRNUM !not sure why, but this had to go to single to work for Mississippi river
  REAL,DIMENSION(LCM)::WVEL,CLEFT,CRIGHT,GRADSED,SEDAVG,CRNUM
  REAL::T1TMP
  INTEGER::L,K,NS
...
!Calculating the morphology before sediment transport of each time step.
  IF(IMORPH_SEDZLJ==1)THEN
      DO L=2,LA
        DO K=1,KBT(L) !accounts for EFDC/SEDZLJ reverse ordering of bed layers
          HBED(L,KB+1-K)=0.01*TSED(K,L)/BULKDENS(K,L)
        ENDDO
      ENDDO
  ENDIF
...
!This is for the changing the hydraulic boundary condition/morphology due to sediment transport.
  IF(IMORPH_SEDZLJ==1)CALL MORPHJ
  DO NS=1,NSED
    DO K=1,KC
      DO L=2,LA
        IF(SED(L,K,NS).LT.-1.0)THEN
         WRITE(6,*)NS,HP(L),IL(L),JL(L),SED(L,K,NS),'NegSed'
          SED(L,K,NS)=0.0  
 !         pause
        ENDIF
      ENDDO
