...
C
 ...
 1000       CLOSE(1001)  
        ...
        IF(ISTOPT(2).EQ.1.OR.ISTOPT(2).EQ.4)THEN 
          ...
          IF(USESHADE)THEN
            ! *** READ IN THE SPATIALLY VARYING INIT T AND BED THICKNESS (DABEDT) 
            PRINT *,
     *        'CALHEAT: READ IN SPATIALLY VARYING SHADE: PSHADE.INP'
 ...
 1010       CLOSE(1001)  
          ELSE
            PRINT *,'CALHEAT: SETTING CONSTANT SHADE TO: 1.0 (NO SHADE)'
          ENDIF
        ENDIF
     ...
     &                  'TAIR_F','FW'
          WRITE(77,998)'TIMEDAY','L','TEM(L,KC)','NETRAD(L,KC)','RAN'
     &                 ,'RB','RE','RC'
  998     FORMAT(A11,8A9)
        ENDIF
      ENDIF
     ...
     &          (1.+0.00412*TEM(L,KC))))  
          ...
          IF(DEBUG)THEN
            IF(TEM(L,KC)<0.0)THEN
              WRITE(77,999)TIMEDAY,L,TEM(L,KC),NETRAD(L,KC),RAN,RB,RE,RC
  999         FORMAT(F11.4,I5,F9.2,5E12.4)
            ENDIF
         ENDIF
...
!            TEM(L,KC)=TEM(L,KC)-TMPKC*CLOUDT(L)*HPI(L)*(TEM(L,KC)
 !    &          -TATMT(L))  
            TEM(L,KC)=TEM(L,KC)-TMPKC*HEQT*HPI(L)*(TEM(L,KC)
     &          -TATMT(L))  !SCJ changed
          ENDDO  
        ENDDO  
