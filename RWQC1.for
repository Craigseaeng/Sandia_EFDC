	...
	IF(ISTRWQ(22) .GT. 0)THEN											
     ...
     &    WQKHPM,WQKHS,WQSTOX,WQKHCO2C,WQKHCO2D,WQKHCO2G,WQKHCO2M
	  WRITE(2,80)'* HALF-SAT. CONSTANT (G/M^3) FOR NUTRIENT UPTAKE '  
        WRITE(2,81)' : (KHNC, KHPC, KHCO2C)       =
     &    ', WQKHNC,WQKHPC,WQKHCO2C  
        WRITE(2,81)' : (KHND, KHPD, KHS, KHCO2D)  = 
     &    ', WQKHND,WQKHPD,WQKHS,WQKHCO2D  
        WRITE(2,81)' : (KHNG, KHPG, KHCO2G)       = 
     &    ', WQKHND,WQKHPG,WQKHCO2G  
        WRITE(2,81)' : (KHNM, KHPM, KHCO2M)       = 
     &    ', WQKHNM,WQKHPM,WQKHCO2M
      ELSE
     ...
     &    WQKHPM,WQKHS,WQSTOX
	  WRITE(2,80)'* HALF-SAT. CONSTANT (G/M^3) FOR NUTRIENT UPTAKE '  
        WRITE(2,81)' : (KHNC, KHPC)       =
     &    ', WQKHNC,WQKHPC  
        WRITE(2,81)' : (KHND, KHPD, KHS)  = 
     &    ', WQKHND,WQKHPD,WQKHS  
        WRITE(2,81)' : (KHNG, KHPG)       = 
     &    ', WQKHND,WQKHPG  
        WRITE(2,81)' : (KHNM, KHPM)       = 
     &    ', WQKHNM,WQKHPM
      ENDIF
...
C ***     MG/L FOR 1-19, TAM-MOLES/L, AND FCB-MPN/L
  ...
  888 FORMAT('    L    K',  
     ...
     &     STOP 'ERROR!! INVALID IWQNPL/NPLFN'  
      ...
      IF(IWQNC.EQ.1)THEN  
        OPEN(3,FILE=NCOFN,STATUS='UNKNOWN')  
        CLOSE(3,STATUS='DELETE')  
        OPEN(3,FILE=NCOFN,STATUS='UNKNOWN')  
      WRITE(3,284)'* NEGATIVE CONCENTRATION OCCURS:'  
        CLOSE(3)  
      ELSE  
  ...
  284 FORMAT(A32, /, 'NAME    ITNWQ    L    I    J    K       CONC')  
      ...
      IF(IWQZ .GT. 1 .AND. IWQKIN .GT. 0)THEN  
        ...
        OPEN(3,FILE='KINETICS.INP',STATUS='UNKNOWN')  
        CALL SKIPCOMM(1,CCMRM)  
        WRITE(2,*) ' '  
        WRITE(2,*) ' SPATIALLY-VARYING KINETICS.INP FILE'  
        WRITE(2,9111)  
        DO I=1,IWQZ  
          READ(3,*) IZ, IWQKA(IZ), WQKRO(IZ), WQKTR(IZ), REAC(IZ),  
...
C  
      ...
      ENDDO  
      IF(IWQZ .GT. 1)THEN  
        OPEN(3,FILE='WQWCMAP.INP',STATUS='UNKNOWN')  
        WRITE(2,999)  
        READ(3,30) (TITLE(M), M=1,3)  
...
C  
      IF(IWQBEN .EQ. 2)THEN  
        DO K=1,2  
          ...
          ENDDO  
        ENDDO  
        OPEN(3,FILE='WQBENMAP.INP',STATUS='UNKNOWN')  
        WRITE(2,999)  
        DO M=1,4  
          READ(3,30) TITLE(M)  
          WRITE(2,30) TITLE(M)  
...
C  
        ...
        IJKC=IC*JC  
        DO M=1,IJKC  
          READ(3,*,END=1112) I, J, XMUD, IZMUD, IZSAND  
          IN=IN+1  
          IF(IJCT(I,J).LT.1 .OR. IJCT(I,J).GT.8)THEN  
...
C  
C     pH PARAMETERS FOR ALGAE ! New card for pH constants. VJ  
C52  
C  

      IF(ISTRWQ(22) .GT. 1)THEN
        ...
        READ(1,*) WQPHC1,WQPHC2,WQPHD1,WQPHD2,WQPHG1,WQPHG2,WQPHM1,		
     &    WQPHM2,WQKPHC1,WQKPHC2,WQKPHD1,WQKPHD2,WQKPHG1,WQKPHG2,
     &    WQKPHM1,WQKPHM2				        
        WRITE(2,*) WQPHC1,WQPHC2,WQPHD1,WQPHD2,WQPHG1,WQPHG2,WQPHM1,		
     &    WQPHM2,WQKPHC1,WQKPHC2,WQKPHD1,WQKPHD2,WQKPHG1,WQKPHG2,
     &    WQKPHM1,WQKPHM2				
