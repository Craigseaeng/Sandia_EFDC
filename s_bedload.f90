SUBROUTINE BEDLOADJ
  ...
  LCM_LOOP_1:DO L=2,LA
     WHERE(D50(1:NSCM)>=200.0.AND.USW(L,1:NSCM)<4.0)
        ...
        ELSEWHERE                              
           BLFLAG(L,1:NSCM)=1
           PSUS(L,1:NSCM)=MAX((LOG(USW(L,1:NSCM))-LOG(SQRT(TCRSUS(1:NSCM))/DWS(1:NSCM)))/(LOG(4.0)-LOG(SQRT(TCRSUS(1:NSCM))/DWS(1:NSCM))),0.0)
        ENDWHERE
     ELSEWHERE
        BLFLAG(L,1:NSCM)=0
        PSUS(L,1:NSCM)=1.0
  ...
  SED_LOOP:DO K=1,NSCM
     LCM_LOOP_2:DO L=2,LA
        ...
        TRANS(L,K)=MAX((TAU(L)-TCRE(K))/TCRE(K),0.0) !eqn. 21
        DZBL(L,K)=MIN(100.*HP(L),D50(K)/10000.0*0.3*DISTAR(K)**0.7*SQRT(TRANS(L,K))) !(eqn. 20b) don't allow bedload height to exceed water column depth
