...
C  
      ...
      ! *** EE BEGIN BLOCK  
	IMPLICIT NONE
	REAL::T1TMP,CORT,CORDTX,HDZBR,DSEDGMMI
...
C  
      IF(ISTRAN(5).GT.0.AND.N.GT.ISEDTIME)CALL CALTOX  
C  
C **  UPDATE TOP BED LAYER THICKNESS AND VOID RATIO  
C **  FOR DEPOSITION-RESUSPENSION STEP  
...
C  
      IF(ISTRAN(5).GT.0.AND.N.GT.ISEDTIME)CALL CALTOXB  
C  
C **  TOXIC CONTAMINANT REACTIONS  
C  
      IF(ISTRAN(5).GE.1.AND.N.GT.ISEDTIME)CALL TOXCHEM  
C
C**********************************************************************C
C
