SUBROUTINE MORPHJ
  ...
  ENDIF
  DO L=2,LA
     DELBED(L)=0.01*SUM(TSED(1:KBT(L),L)/BULKDENS(1:KBT(L),L))-HBEDA(L)
     BELV1(L)=BELV(L)  
     HTMP(L)=HP(L)  
     H1P(L)=HP(L)  
     P1(L)=P(L)
     HBEDA(L)=0.01*SUM(TSED(1:KBT(L),L)/BULKDENS(1:KBT(L),L))
     DO K=1,KBT(L) !accounts for reverse numbering between EFDC and SEDZLJ
