...
C
        ...
        DO L=2,LA  
          !SBX(L)=0.5*SUB(L)*DYU(L)    !See "EFDC Buoyancy Issues.doc" for this correction
...
!     &                +(BE(L)+BE(L-1))*(BELV(L)-BELV(L-1)))  
     ...
     &                +(BE(L)+BE(L-1))*(BELV(L)-BELV(L-1)))  
        ...
        DO L=2,LA  
          LS=LSC(L)  
          !SBY(L)=0.5*SVB(L)*DXV(L)  !See "EFDC Buoyancy Issues.doc" for this correction 
...
C **  HOST-GUEST CHANNEL INTERACTION FOR NON BOUNDARY POINTS  
C  
      DO L=2,LA
        LN=LNC(L)
