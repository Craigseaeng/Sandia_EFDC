...
C  
      IF(NQSIJ.GT.0)THEN  
C24*  READ VOLUME SOURCE/SINK LOCATIONS, MAGNITUDES, & VOL & CONC SERIES !Note: NQSMF is never used in the code
        NCARD='24'  
        CALL SEEK('C24')  
        DO L=1,NQSIJ  
...
C FRACTION OF POC ASSIGNED TO EACH SEDIMENT CLASS IN WATER COLUMN
C  
        DO NT=1,NTOX  
...
C     DHC
      IF(ISWAVE.EQ.3.OR.ISPD.GE.2)THEN
        ! *** COMPUTE CELL AREAS AND CENTROIDS, REQUIRES CORNERS.INP FILE
        CALL AREA_CENTRD
      ENDIF
      
      IF (ISPD.GE.2) CALL DRIFTERINP
C  
...
C  
        ...
        ELSE  
          DO LT=1,LVC  
            ...
            ZBR(L)=ZBRADJ + ZBRCVRT*ZBRIJ  
            MVEGL(L)=MVEGIJT
		    IF(MVEGIJT>90)THEN !SCJ IF STATEMENT
              ITURB=ITURB+1
		      IJLTURB(ITURB,1)=I
		      IJLTURB(ITURB,2)=J
...
C  
     ...
     &        SWRATNS,FSWRATF,DABEDT,TBEDIT,HTBED1,HTBED2  
          IF(ISO.GT.0) GOTO 940
          IF(MAX(WQKHCO2C,WQKHCO2D,WQKHCO2G,WQKHCO2M)<0.0)THEN !SCJ read in pH values from aser.inp last column
            DO M=1,MASER(N)  
...
C  
     ...
     &        ALPVEG(M),BETVEG(M),GAMVEG(M),SCVEG(M)  
        ...
        ENDIF
	!!!BEGIN SCJ BLOCK
	  IF(MAXVAL(MVEGL(2:LA))>90)THEN !MHK devices
        PRINT *,'READING MHK.INP'
        OPEN(1,FILE='MHK.INP',STATUS='UNKNOWN')  
        DO NS=1,29  
          READ(1,*) !skip 29 header lines
        ENDDO  
        READ(1,*,ERR=3122)MHKTYP,NFLAGPWR,UPSTREAM,OUTPUTFLAG
        IF(MHKTYP/=MVEGTYP)PRINT*,'MHKTYP/=MVEGTYP',pause
        ALLOCATE(BOFFMHK(MHKTYP),BOFFSUP(MHKTYP))
        ALLOCATE(TOFFMHK(MHKTYP),TOFFSUP(MHKTYP))
        IF(NFLAGPWR==1)THEN
          DO M=1,MHKTYP  !read 1 line per MHK tupe
            READ(1,*,ERR=3122)WIDTHMHK(M),WIDTHSUP(M),
     &BOFFMHK(M),BOFFSUP(M),TOFFMHK(M),TOFFSUP(M),
     &CTMHK(M),CDSUP(M),VMINCUT(M),VMAXCUT(M),DENMHK(M)
		    CTMHK(M)=CTMHK(M)*DENMHK(M)
		    CDSUP(M)=CDSUP(M)*DENMHK(M)
		    DO L=2,LA
		      IF(M+90==MVEGL(L))THEN !set min/max elevations
        ...
        ELSEIF(NFLAGPWR==3)THEN !FFP input style
          READ(1,*,ERR=3122)WIDTHMHK(M),WIDTHSUP(M),
     &BOFFMHK(M),HEIGHTMHK(M),HEIGHTSUP(M),REFELEV(M),
     &CTMHK(M),CDSUP(M),VMINCUT(M),VMAXCUT(M),DENMHK(M)
		  CTMHK(M)=CTMHK(M)*DENMHK(M)
		  CDSUP(M)=CDSUP(M)*DENMHK(M)
		  DO L=2,LA
        ...
        CLOSE(1)    
 	    DO L=2,LA
	      IF(MVEGL(L)>90)THEN
		    IF((DIAMMHK>DXP(L).OR.DIAMMHK>DYP(L))
     &.AND.DENMHK(MVEGL(L)-90)>1.0)THEN !error check
