...
C **  IN SEDIMENT BED FOR HOT AND COLD START CONDITIONS  
C     CHANGE RECORD  
...
C  
      IF(IHOTSTRT.NE.0.OR.IWRSP(1).GE.98)THEN  
C  
C **  SET POROSITY  
C  
...
C  
     ...
     &           +0.001*(SEDBT(L,K)+SNDBT(L,K))/HBED(L,K)  
        ...
        IF(IWRSP(1).LT.98)THEN
          DO K=1,KB  
            DO L=2,LA  
              IF(HBED1(L,K).GT.0.0)THEN  
                BDENBED1(L,K)=1000.0*PORBED1(L,K)  
...
C  
      IF(IBMECH.EQ.0.AND.IWRSP(1).LT.98) THEN  
C  
...
C  
     ...
     &                  +1000.0*SSG(NS)*SNDB(L,K,NX)  
            ...
            DO K=1,KB  
              DO L=2,LA  
                BDENBED(L,K)=1000.0*PORBED(L,K)  
     &              +(1.0-PORBED(L,K))*BDENBED(L,K)  
                BDENBED1(L,K)=BDENBED(L,K)  
              ENDDO  
            ENDDO  
            DO K=1,KB  
              DO L=2,LA  
                SEDBT(L,K)=1000.0*HBED(L,K)*(BDENBED(L,K)  
     &              -1000.0*PORBED(L,K))  
              ENDDO  
            ENDDO  
            IF(ISTRAN(6).GE.1)THEN  
...
C **  SET COHESIVE BED CRITICAL STRESSES AND RESUSPENSION RATES  
C  
        IF(ISTRAN(6).GE.1.AND.IWRSP(1).LT.98)THEN  
          IF(IWRSP(1).EQ.0)THEN  
            DO K=1,KB  
              DO L=2,LA  
                TAURS(L,K)=TAUR(1)  
...
C **  BED AND REINITIALIZE IF NECESSARY  
C  
...
C  
 1000 CONTINUE
      IF(DEBUG.AND.IWRSP(1)/=98)THEN !No diagnostics for SEDZL
